...
function DateFormat({date, isDark=false, isHeading=false}: {date: Date, isHeading?: boolean, isDark?: boolean}) {
...
`
    return (
        <div>
            {isHeading ? 
                <Heading size="h2" styles={isDark ? {weight: "font-semibold", color: "text-gray-950"} : {weight: "font-semibold"}}>
                    {day}
                </Heading> :
                <Text size='lg' styles={isDark ? {weight: 'font-medium', color: "text-gray-950"} : {weight: 'font-medium'}}>
                    {day}
                </Text>
            }
        </div>
